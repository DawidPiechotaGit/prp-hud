...
const store = () => {
  ...
  function getLocalStorage(
    key: iconNamesKind | keyof playerStatusType,
    fallback: any
  ) {
  ...
  function getDefaultSettings(): playerStatusType {
    return {
      designMode: false,
      designProgress: 0,
      globalIconSettings: getLocalStorage(
        "globalIconSettings",
        (({ isShowing, name, icon, progressValue, ...o }) => o)(
          defaultHudIcon()
        )
      ),
      icons: {
        voice: getLocalStorage(
          "voice",
          defaultHudIcon("voice", true, faMicrophone)
        ),
        health: getLocalStorage(
          "health",
          defaultHudIcon("health", false, faHeartPulse)
        ),
        armor: getLocalStorage(
          "armor",
          defaultHudIcon("armor", false, faShieldAlt)
        ),
        hunger: getLocalStorage(
          "hunger",
          defaultHudIcon("hunger", false, faDrumstickBite)
        ),
        thirst: getLocalStorage(
          "thirst",
          defaultHudIcon("thirst", false, faGlassWater)
        ),
        ...
        oxygen: getLocalStorage(
          "oxygen",
          defaultHudIcon("oxygen", false, faLungs)
        ),
        armed: getLocalStorage(
          "armed",
          defaultHudIcon("armed", false, faPersonRifle)
        ),
        parachute: getLocalStorage(
          "parachute",
          defaultHudIcon("parachute", false, faParachuteBox)
        ),
        ...
        harness: getLocalStorage(
          "harness",
          defaultHudIcon("harness", false, faUserSlash)
        ),
        cruise: getLocalStorage(
          "cruise",
          defaultHudIcon("cruise", false, faTachometerAltFast)
        ),
        nitro: getLocalStorage(
          "nitro",
          defaultHudIcon("nitro", false, faMeteor)
        ),
  ...
  const methods = {
    ...
    updateIconSetting(
      ...
      settingName: keyof optionalHudIconType,
      value: any
    ) {
      update((state) => {
    ...
    updateShowingDynamicIcon(iconName: iconNamesKind, staticShow: boolean) {
      ...
      update((state) => {
        switch (iconName) {
          case "armor":
            state.icons.armor.isShowing = methods.staticGenericZeroHandleShow(
              staticShow,
              state.icons.armor.progressValue
            );
            result = state.icons.armor.isShowing;
            break;
          case "engine":
            state.icons.engine.isShowing = methods.staticEngineHandleShow(
              staticShow,
              state.icons.engine.progressValue
            );
            result = state.icons.engine.isShowing;
            break;
          case "health":
            state.icons.health.isShowing =
              methods.staticGenericHundredHandleShow(
                staticShow,
                state.icons.health.progressValue
              );
            result = state.icons.health.isShowing;
            break;
          case "hunger":
            state.icons.hunger.isShowing =
              methods.staticGenericHundredHandleShow(
                staticShow,
                state.icons.hunger.progressValue
              );
            result = state.icons.hunger.isShowing;
            break;
          case "nitro":
            state.icons.nitro.isShowing = methods.staticNitroHandleShow(
              staticShow,
              state.icons.nitro.progressValue,
              state.icons.engine.progressValue
            );
            result = state.icons.nitro.isShowing;
            break;
          case "oxygen":
            state.icons.oxygen.isShowing =
              methods.staticGenericHundredHandleShow(
                staticShow,
                state.icons.oxygen.progressValue
              );
            result = state.icons.oxygen.isShowing;
            break;
          case "stress":
            state.icons.stress.isShowing = methods.staticGenericZeroHandleShow(
              staticShow,
              state.icons.stress.progressValue
            );
            result = state.icons.stress.isShowing;
            break;
          case "thirst":
            state.icons.thirst.isShowing =
              methods.staticGenericHundredHandleShow(
                staticShow,
                state.icons.thirst.progressValue
              );
    ...
    updateAllShowingDynamicIcons(val: boolean) {
      update((state) => {
        for (const icon in state.dynamicIcons) {
          ...
          state.icons[icon].isShowing = methods.updateShowingDynamicIcon(
            icon as dynamicIconNamesKind,
            val
          );
        }
        return state;
    ...
    receiveStatusUpdateMessage(data: playerHudUpdateMessageType) {
      update((state) => {
        ...
        state.icons.parachute.progressValue = capAmountToHundred(
          data.parachute
        );
        ...
        state.icons.health.isShowing = methods.staticGenericHundredHandleShow(
          state.dynamicIcons.health,
          state.icons.health.progressValue
        );
        ...
        state.icons.hunger.isShowing = methods.staticGenericHundredHandleShow(
          state.dynamicIcons.hunger,
          state.icons.hunger.progressValue
        );
        ...
        state.icons.thirst.isShowing = methods.staticGenericHundredHandleShow(
          state.dynamicIcons.thirst,
          state.icons.thirst.progressValue
        );
        ...
        state.icons.stress.isShowing = methods.staticGenericZeroHandleShow(
          state.dynamicIcons.stress,
          state.icons.stress.progressValue
        );

        state.icons.oxygen.isShowing = methods.staticGenericHundredHandleShow(
          state.dynamicIcons.oxygen,
          state.icons.oxygen.progressValue
        );

        state.icons.engine.isShowing = methods.staticEngineHandleShow(
          state.dynamicIcons.engine,
          state.icons.engine.progressValue
        );
        ...
        for ([key, value] of Object.entries(data)) {
          state.icons[key] = {
            ...createShapeIcon(value.shape, {
              ...
              isShowing: state.icons[key].isShowing,
              name: state.icons[key].name,
